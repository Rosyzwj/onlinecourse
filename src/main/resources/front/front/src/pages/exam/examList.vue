<template>
	<div :style='{"border":"1px solid #e5e9ef","padding":"20px","margin":"10px auto","borderRadius":"8px","background":"#fff","width":"1200px","position":"relative","box-shadow":"0 1px 2px rgba(0,0,0,0.05)"}'>
		...
		<div class="section-title" :style='{"padding":"15px 0","margin":"0px 0 20px","color":"#333","borderRadius":"6px","textAlign":"center","background":"#f5f7fa","fontSize":"18px","lineHeight":"24px","fontWeight":"600","borderBottom":"2px solid #3498db"}'>考试成绩</div>
		<el-table
		  :data="tableData"
		  ...
		  <el-table-column
			label="考试得分">
			<template slot-scope="scope">
				<div :style='{"fontWeight":"500","color":"#e74c3c"}'>{{ scope.row.myscore || 0 }}分</div>
			</template>
		  </el-table-column>
	  ...
	  export default {
		...
		methods: {
		  ...
		  getExamList(page) {
			...
			if (!userTable) {
			  this.errorMsg = "UserTableName未找到";
				...
				.then(res => {
				  if (res.data.code === 0) {
					...
					if (this.tableData.length === 0) {
					  this.errorMsg = "暂无考试记录";
					}
				  } else {
					this.errorMsg = res.data.msg || "获取考试记录失败";
					this.$message.error(this.errorMsg);
				  }
				})
				.catch(examError => {
				  console.error("获取考试记录失败", examError);
				  this.errorMsg = "获取考试记录失败，请稍后再试";
				  this.$message.error(this.errorMsg);
				})
				.finally(() => {
				  this.loading = false;
