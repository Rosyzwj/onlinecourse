			...
			<el-tab-pane label="评论" name="second">
				...
				<div v-if="infoList.length" :style='{"padding":"15px","flexWrap":"wrap","display":"flex"}' class="comment">
					<div :style='{"padding":"0","margin":"0 20px 20px","borderColor":"#ddd","overflow":"hidden","alignItems":"center","borderWidth":"0 0 1px 0","background":"#fcfcfc","width":"45%","borderStyle":"solid","height":"auto"}' v-for="item in infoList" :key="item.id">
						<div class="student" :style='{"width":"100%","alignItems":"center","background":"#f5f5fc","display":"flex","height":"auto"}'>
							...
							<el-image v-if="!item.avatarurl" :style='{"width":"50px","margin":"0 10px 0 0","borderRadius":"0","objectFit":"cover","height":"50px"}' :size="50" :src="require('@/assets/profile.png')"></el-image>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscusscourse/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscusscourse/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
