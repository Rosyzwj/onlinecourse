<template>
	<div>
		...
		<div class="list-preview" :style='{"width":"1200px","margin":"10px auto","position":"relative","background":"none","z-index":"1"}'>
			<div class="category-1" :style='{"padding":"15px 20px","borderColor":"#e6eaf2","borderRadius":"10px","background":"#f8f9fa","borderWidth":"1px","display":"flex","flexWrap":"wrap","width":"100%","borderStyle":"solid","height":"auto","gap":"10px","alignItems":"center"}'>
				...
				<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in sort" :key="index" @click="getList(1, item, 'btn' + index)" :ref="'btn' + index" plain>{{item}}</div>
			</div>
		...
		<el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #e6eaf2","padding":"15px 20px","margin":"15px 0 0 0","alignItems":"center","borderRadius":"10px","flexWrap":"wrap","background":"#f8f9fa","display":"flex","width":"100%","height":"auto","gap":"15px"}'>
		  ...
		  <el-form-item :style='{"margin":"0"}'>
			<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"40px","display":"inline-block","color":"#666","fontWeight":"500"}'>资料名称</div>
			<el-input v-model="formSearch.resourcename" placeholder="资料名称" clearable></el-input>
		  </el-form-item>
		  <el-form-item :style='{"margin":"0"}'>
			<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"40px","display":"inline-block","color":"#666","fontWeight":"500"}'>资料类别</div>
			<el-select v-model="formSearch.resourcesort" placeholder="请选择资料类别" :clearable="true">
			  <el-option v-for="(item, index) in resourcesortOptions" :key="index" :label="item" :value="item"></el-option>
			</el-select>
		  </el-form-item>
		  <el-button v-if=" true " :style='{"cursor":"pointer","border":"none","padding":"0px 20px","margin":"0","outline":"none","color":"#fff","borderRadius":"6px","background":"#3498db","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px","transition":"all 0.3s ease"}' type="primary" @click="getList(1, curSort)"><i v-if="true" :style='{"color":"#fff","margin":"0 8px 0 0","fontSize":"14px"}' class="el-icon-search"></i>查询</el-button>
		  <el-button v-if="isAuth('resource','新增')" :style='{"cursor":"pointer","border":"none","padding":"0px 20px","margin":"0","outline":"none","color":"#fff","borderRadius":"6px","background":"#3498db","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px","transition":"all 0.3s ease"}' type="primary" @click="add('/index/resourceAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 8px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
		</el-form>
			...
			<div class="list2 index-pv1" :style='{"width":"100%","padding":"25px 20px","flexWrap":"wrap","justifyContent":"flex-start","display":"flex","height":"auto","gap":"20px"}'>
				<div :style='{"cursor":"pointer","padding":"0","borderRadius":"10px","background":"#fff","display":"inline-block","width":"calc(25% - 15px)","fontSize":"0","position":"relative","height":"auto","border":"1px solid #f0f0f0","overflow":"hidden","transition":"all 0.3s ease"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
					...
					<div class="item-info" :style='{"padding":"15px","overflow":"hidden","background":"#fff","display":"inline-block","width":"100%","height":"auto"}'>
						<div :style='{"margin":"0 0 8px 0","overflow":"hidden","color":"#2c3e50","fontWeight":"500","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","whiteSpace":"nowrap"}' class="name ">{{item.resourcename}}</div>
	...
	<script>
	  export default {
		...
		created() {
		  ...
		  this.$http.get('option/resourcesort/resourcesort').then(res => {
			if (res.data.code == 0) {
		...
		methods: {
		  ...
		  getList(page, sort, ref = '') {
			if(sort == '全部') this.swiperIndex = -1;
			...
			if (this.formSearch.resourcename != '') searchWhere.resourcename = '%' + this.formSearch.resourcename + '%';
			if (this.formSearch.resourcesort != '') searchWhere.resourcesort = this.formSearch.resourcesort;
			if (this.curSort != '全部') searchWhere.resourcesort = this.curSort;
			this.$http.get('resource/list', {params: Object.assign(params, searchWhere)}).then(res => {
			  if (res.data.code == 0) {
		  ...
		  curChange(page) {
			this.getList(page,this.curSort);
		  },
		  prevClick(page) {
			this.getList(page,this.curSort);
		  },
		  sizeChange(size){
			this.pageSize = size
			this.getList(1,this.curSort);
		  },
		  nextClick(page) {
			this.getList(page,this.curSort);
		  },
		  toDetail(item) {
			this.$router.push({path: '/index/resourceDetail', query: {detailObj: JSON.stringify(item)}});
		  },
		}
	  }
	</script>
