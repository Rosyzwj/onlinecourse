<template>
	<div :style='{"border":"1px solid #e5e9ef","padding":"20px","margin":"10px auto","borderRadius":"8px","background":"#fff","width":"1200px","position":"relative","box-shadow":"0 1px 2px rgba(0,0,0,0.05)"}'>
		...
		<div class="section-title" :style='{"padding":"15px 0","margin":"0px 0 20px","color":"#333","borderRadius":"6px","textAlign":"center","background":"#f5f7fa","fontSize":"18px","lineHeight":"24px","fontWeight":"600","borderBottom":"2px solid #3498db"}'>我的发布</div>
		<el-table
		  :data="tableData"
		  ...
		  <el-table-column
			...
			<template slot-scope="scope">
			  <div :style='{"fontWeight":"500","color":"#3498db","transition":"color 0.2s ease","cursor":"pointer"}' @mouseover="titleHover=true" @mouseout="titleHover=false">
				{{ scope.row.title }}
			  </div>
			</template>
		  </el-table-column>
	...
	<script>
	  export default {
		...
		methods: {
		  ...
		  getMyForumList(page) {
			this.$http.get('forum/list', {params: {page, limit: this.pageSize, parentid: 0, sort: 'addtime', order: 'desc', mark: 1 }}).then(res => {
			  if (res.data.code == 0) {
