	...
	<div class="addEdit-block" :style='{"padding":"0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0.5% 20px","display":"inline-block"}' class="textarea" v-if="type!='info'" label="评论内容" prop="content">
					<el-input
				...
				<el-form-item :style='{"width":"48%","margin":"0 0.5% 20px","display":"inline-block"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussresource/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussresourceCrossAddOrUpdateFlag = false;
						 this.parent.search();
