<template>
	<div class="addEdit-block" :style='{"padding":"0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"48%","margin":"0 0.5% 20px","display":"inline-block"}' class="input" v-if="type!='info'"  label="课程类别" prop="coursesort">
					<el-input v-model="ruleForm.coursesort" placeholder="课程类别" clearable  :readonly="ro.coursesort"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0.5% 20px","display":"inline-block"}' v-else class="input" label="课程类别" prop="coursesort">
					<el-input v-model="ruleForm.coursesort" placeholder="课程类别" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `coursesort/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.coursesortCrossAddOrUpdateFlag = false;
						 this.parent.search();
