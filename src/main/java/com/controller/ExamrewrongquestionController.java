    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(examrewrongquestion !=null){
            ...
            //级联表 学生
            //级联表
            StudentEntity yonghu = studentService.selectById(examrewrongquestion.getYonghuId());
            if(yonghu != null){
    ...
    */
    ...
    public R save(@RequestBody ExamrewrongquestionEntity examrewrongquestion, HttpServletRequest request){
        ...
        if(false)
            ...
            examrewrongquestion.setYonghuId(Long.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//            .notIn("examrewrongquestion_types", new Integer[]{102})
    ...
    public R statistics(@RequestParam Map<String, Object> params, HttpServletRequest request){
        logger.debug("statistics方法:,,Controller:{},,params:{}",this.getClass().getName(),JSONObject.toJSONString(params));
        String role = String.valueOf(request.getSession().getAttribute("role"));
        
        // 只有教师可以查看统计（兼容"教师"和"老师"两种角色名称）
        if(!"教师".equals(role) && !"老师".equals(role)) {
            return R.error(511, "只有教师可以查看错题统计");
        }
        ...
        EntityWrapper<ExamrewrongquestionEntity> wrapper = new EntityWrapper<>();
        List<ExamrewrongquestionEntity> allWrongQuestions = examrewrongquestionService.selectList(wrapper);
        
        // 统计各试卷的错题数
        Map<Integer, Integer> examPaperCount = new HashMap<>();
        Map<Integer, Integer> questionTypeCount = new HashMap<>();
        
        for(ExamrewrongquestionEntity wrong : allWrongQuestions) {
            // 按试卷统计
            Integer paperId = wrong.getExampaperId();
            examPaperCount.put(paperId, examPaperCount.getOrDefault(paperId, 0) + 1);
            
            // 按试题类型统计
            ExamquestionEntity question = examquestionService.selectById(wrong.getExamquestionId());
            if(question != null) {
                Integer questionType = question.getExamquestionTypes();
        ...
        for(Map.Entry<Integer, Integer> entry : questionTypeCount.entrySet()) {
            ...
            switch(entry.getKey()) {
                ...
                case 2: typeName = "多选题"; break;
                case 3: typeName = "判断题"; break;
                case 4: typeName = "填空题"; break;
        ...
        for(ExamrewrongquestionEntity wrong : allWrongQuestions) {
            Integer questionId = Math.toIntExact(wrong.getExamquestionId());
            questionCount.put(questionId, questionCount.getOrDefault(questionId, 0) + 1);
        }
        
        List<Map.Entry<Integer, Integer>> sortedQuestions = new ArrayList<>(questionCount.entrySet());
        sortedQuestions.sort((a, b) -> b.getValue().compareTo(a.getValue()));
        
        List<Map<String, Object>> topWrongQuestions = new ArrayList<>();
        int limit = Math.min(10, sortedQuestions.size());
        for(int i = 0; i < limit; i++) {
            Map.Entry<Integer, Integer> entry = sortedQuestions.get(i);
            ExamquestionEntity question = examquestionService.selectById(entry.getKey());
            if(question != null) {
                Map<String, Object> item = new HashMap<>();
                item.put("questionId", entry.getKey());
                item.put("questionName", question.getExamquestionName());
                item.put("wrongCount", entry.getValue());
                topWrongQuestions.add(item);
            }
        }
        result.put("topWrongQuestions", topWrongQuestions);
        
        // 3. 错题总数
        result.put("totalWrongCount", allWrongQuestions.size());
        
        // 4. 涉及学生数
        Set<Long> studentSet = new HashSet<>();
        for(ExamrewrongquestionEntity wrong : allWrongQuestions) {
            studentSet.add(wrong.getYonghuId());
        }
        result.put("studentCount", studentSet.size());
        
        // 5. 涉及试卷数
        result.put("examPaperCount", examPaperCount.size());
