...
public class ExamrecordController {
    ...
    public R page2(@RequestParam Map<String, Object> params,ExamrecordEntity examrecord, HttpServletRequest request){
        ...
        // 只有学生才限制只能看到自己的考试记录，教师和管理员可以看到所有学生的考试记录
        if("学生".equals(role)) {
            examrecord.setYonghuId((Long)request.getSession().getAttribute("userId"));
        }
    ...
    public R page2(@PathVariable("id")  Long id){
        List<ExamRecord> list =  examrecordService.selectExamRecordWithRank(id);
    ...
    */
    ...
    public R info(@PathVariable("id") String id, HttpServletRequest request){
        ...
        if(examrecord !=null){
            ...
            //级联表 学生
            //级联表
            StudentEntity yonghu = studentService.selectById(examrecord.getYonghuId());
            if(yonghu != null){
    ...
    */
    ...
    public R save(@RequestBody ExamrecordEntity examrecord, HttpServletRequest request){
        ...
        if(false)
            ...
            examrecord.setYonghuId(Long.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
